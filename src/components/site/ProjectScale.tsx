...
export function ProjectScale() {
  ...
  return (
    <section
      ...
      className="relative bg-[#0B0D0F] text-white py-24 sm:py-32 lg:py-40 border-b border-white/10 overflow-hidden"
    >
      {/* Background Architectural Project Backdrop with Cinematic Contrast */}
      <div className="absolute inset-0 z-0">
        <picture>
          ...
          <img
            ...
            className="size-full object-cover object-center brightness-[0.20] contrast-[1.15]"
            loading="lazy"
          />
        </picture>
        <div className="absolute inset-0 bg-gradient-to-t from-[#0B0D0F] via-[#0B0D0F]/80 to-[#0B0D0F]" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#0B0D0F]/95 via-[#0B0D0F]/85 to-[#0B0D0F]/70" />
      </div>
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mb-16 sm:mb-20">
          <h2 className="font-editorial-title text-3xl sm:text-5xl lg:text-6xl font-extrabold uppercase text-white leading-[1.05]">
            BUILT ACROSS <br />
            <span className="text-[#B08A4A]">INDUSTRIES.</span>
          </h2>
          <p className="mt-4 text-base sm:text-lg text-[#C8CCD0] font-sans leading-relaxed">
        ...
        {/* 4 Architectural Metric Panels with 21st.dev NumberTicker */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {metrics.map((m, idx) => (
            <div
              key={idx}
              className={`p-6 sm:p-8 bg-[#14171A]/90 border border-white/10 backdrop-blur-sm transition-all duration-700 ${
                inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
              style={{ transitionDelay: `${idx * 150}ms` }}
            >
              <div className="font-mono text-xs text-[#8C9398] mb-3 flex items-center justify-between">
                <span>METRIC 0{idx + 1}</span>
                <span className="size-1.5 bg-[#B08A4A] rounded-full" />
              </div>

              <div className="font-editorial-title text-4xl sm:text-5xl font-extrabold text-white tracking-tight mb-2">
                <span className="text-[#B08A4A]">
        ...
        {/* Footer Ledger Note */}
        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 font-mono text-xs text-[#8C9398]">
          <div>
            IS 2062 Prime Mild Steel · IS 800 Structural Compliance · Noida Yard Direct
          </div>
          <Link
            to="/projects"
            className="inline-flex items-center gap-1.5 text-[#B08A4A] hover:text-white font-bold transition-colors"
          >
